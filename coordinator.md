# Herdr + Pi Coordinator Operating Card v7.30-draft
...
- Codex sessions default to the normal, non-Fast service tier. Enable Fast only when the owner explicitly requests it for that specific task or session; Fast authorization for a producer does not carry over to reviewers or other roles. On every dispatch or resume, explicitly select the intended native tier and verify the live setting before work; never inherit a prior session's Fast default without authorization. Keep model and reasoning effort independently bound to the assigned route.
