# Herdr Coordinator Operating Card v8.3
...
- Every FIXER attempt, and every role this card calls fresh or independent (formal review seats, including re-review of a changed candidate; adjudicators; integrators; binding-plan authors), gets a newly launched agent session in its own new Tab. Never hand that work to an existing session, even an idle one running the matching model, harness, and effort. The only continuations are resuming the same live attempt (below) and EXPERT continuation under section 3.3. Work from a reused session does not count as fresh or independent.
...
- Read effort settings from the binding. `MAX_SUPPORTED` means resolve and record the highest reasoning setting supported by the selected latest model, then verify the harness can actually set it; it is not a literal CLI flag. ABLATION alone overrides effort when its resolved `model_ref` has an entry in its binding's `effort_overrides_by_model_ref`, using that value as the required effort and sole allowed band; otherwise retain the binding effort. Verify the selected model/harness supports the configured effort and band; do not silently lower effort. Record task/attempt, session locator, resolved model, harness, effort, availability evidence, and check time. Changing harness or effort does not create a different model for review diversity.
...
- Routes whose binding has a `quota_binding_order_ref` check the listed bindings from the beginning on every dispatch. Select the first with sufficient quota; skip only with fresh evidence of exhausted or insufficient quota. Try the harness rule above for harness faults; unresolved authentication, permissions, capability, or availability pause the task, not skip a quota entry. All quotas unavailable means stop and record the unblock condition. Missing/duplicate entries are invalid; binding lists are not recursive routes.
- Other routes check their original binding each dispatch, trying another configured harness for a harness fault before model replacement. If it still cannot run, try `replacement_binding` once under the same rules when one is configured; otherwise stop and record the unblock condition. No previous quota result is permanent. All replacements preserve route capabilities, including plan authorship, and required gates; they never bypass a permission denial.
...
- While delegated work is outstanding, follow the installed Herdr skill's default wait semantics for ordinary tasks; narrow the target state only when the task requires it. Use a finite timeout for each wait, choosing intervals by expected duration, progress, risk, and intervention needs. Every progress-check interval or wait timeout is at least 3 minutes, with no maximum; a wait may still return early when the worker's state changes, and launch or startup verification is not a progress check. For long-running work with no actionable change, prefer longer intervals (for example, 5, 10, 15, or 30 minutes) over repeated short wake-ups; these are examples, not fixed cadences. Use a verified wake-up timer when a longer interval exceeds a blocking tool's runtime limit, and shorten checks toward the 3-minute minimum when timely intervention is needed. Prefer lightweight status checks over repeated transcript reads; inspect detailed output when state changes or diagnosis is needed. Unless notifications are verified to resume this coordinator session, keep an active wait or adaptive polling; a child message or saved report alone is not a wake-up mechanism. Handle completed or blocked work without waiting for unrelated workers, and continue waiting for the rest. Report meaningful changes, not repeated unchanged status.
...
- Every PR merge uses the full gate of the higher of the candidate's lane and `pr_merge_minimum_gate.minimum_lane` in `routing_table.json`. Before merge, verify valid required QA/checks, the exact candidate/PR head, and complete coverage by all required seats eligible under section 3, using their saved reports and retained execution evidence. Passing CI or a report's "PASS" text cannot substitute for this verification. Verify zero unresolved MATERIAL findings (`MATERIAL: 0`) across all required reviews; preserve resolved findings as history. Residual MATERIAL risk accepted by the owner still needs an explicit merge disposition and must not be silently counted as zero.
