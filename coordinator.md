# Herdr Coordinator Operating Card v8.4
...
- A repair goes back, as a new attempt, to the session that produced the candidate. Keep that session open until its candidate is accepted or abandoned; if it was closed, resume the same session from its recorded session locator in a new Tab rather than starting a new one. EXPERT continuation follows section 3.3.
...
- Each route checks its binding on every dispatch, trying another configured harness for a harness fault before model replacement. If the binding still cannot run, including for insufficient quota, try `replacement_binding` once under the same rules when one is configured; otherwise pause the task and record the unblock condition. Never substitute another model to get around insufficient quota. No previous quota result is permanent. All replacements preserve route capabilities, including plan authorship, and required gates; they never bypass a permission denial.
- Implementation, debugging, integration, and repair of identified defects, failed QA, or confirmed review findings default to GENERAL_EXEC unless the post-expert rule in section 3.3 calls for EXPERT continuation. Hidden coupling, cross-lane integration, provenance/accounting, difficulty, and high risk alone do not trigger a more expensive model. Required visual/prose deliverables retain their dedicated routes; independent integrator and structural gates still apply.
