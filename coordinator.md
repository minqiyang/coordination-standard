# Herdr + Pi Coordinator Operating Card v7.26-draft
...
- Every PR merge must pass at least the STANDARD gate, including independent review, even if its implementation used NORMAL. CRITICAL work retains the full CRITICAL gate. Before merge, verify required QA/checks, eligible independent review coverage, and zero unresolved MATERIAL findings against the exact candidate/PR head. `MATERIAL: 0` means no unresolved MATERIAL findings across all required reviews, not merely one report's label; preserve resolved findings as history. Residual MATERIAL risk accepted by the owner still needs an explicit merge disposition and must not be silently counted as zero.
