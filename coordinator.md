...
- A repair goes back, as a new attempt, to the session that produced the candidate. Keep that session open until its candidate is accepted or abandoned; if it was closed, resume the same session from its recorded session locator in a new Tab rather than starting a new one. If that session's context is close to exhausted, a new session may take over the repair, handed off through the original session's reports and evidence. A new candidate lineage or a new stage (for example, the next binding-plan phase) starts a new session. EXPERT continuation follows section 3.3.
...
Choose the highest applicable lane and record a brief risk reason. Use CRITICAL whenever its risk criteria apply; otherwise use ELEVATED for important changes and ROUTINE for ordinary work. If ROUTINE eligibility is uncertain, use at least ELEVATED while clarifying the risk. Mechanical work is not automatically low risk; classify its actual effects. Read-only analysis, research, and assessment that produce no candidate default to ROUTINE; binding plans (section 7) and analysis the owner designates as a gate input keep their own lane.
